...
- `exec` is durable: the mutation fast-returns; long-running commands stream output over graphql-ws (`sandboxExecOutput` subscription, cursor-resumable across the server's ~15m subscription cap). `exec()` returns an `ExecHandle` (await it for `ExecResult`; `.execId`/`.kill()`); `exec({ execId })` reattaches. `timeoutSec` is enforced client-side (kill + `timedOut: true`). Live e2e: `SANDBOX_E2E=1 pnpm vitest run tests/exec.e2e.test.ts` with `.env` creds.
