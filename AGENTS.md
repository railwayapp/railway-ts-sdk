...
- Release: run the `Create Release` workflow (Actions → workflow_dispatch, `bump: patch`). It bumps `package.json`, commits to `main`, tags, and pushes; the tag push triggers `release.yml` to publish to npm. Do NOT hand-push a tag off `main` — `npm publish` uses the `package.json` version at the tagged commit, so a tag without a matching version bump fails with "cannot publish over previously published version".
