...
- Keep public SDK scope minimal: `Sandbox.create`, `exec`/`fork`/`destroy`/`refresh` on the instance, `connect`/`list` statics, `await using` auto-destroy. `fork` reuses the `sandboxCreate` mutation via `sourceSandboxId` (`create(source)` is the static form). Files, ports, and streaming are future.
