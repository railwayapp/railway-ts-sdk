import path from "node:path";
import { pathToFileURL } from "node:url";
import { graphToEnvironmentConfig, projectDefinitionToGraph } from "./compiler.js";
import type { CompileResult, GraphCompileOptions, ProjectDefinition } from "./graph.js";
import { createRailwayContext, project as projectFactory, type RailwayProgram } from "./sdk.js";

export async function evaluateRailwayFile(filePath: string, options: GraphCompileOptions = {}): Promise<CompileResult> {
  const absolutePath = path.resolve(filePath);
  const mod = await import(`${pathToFileURL(absolutePath).toString()}?t=${Date.now()}`) as {
    default?: RailwayProgram | ProjectDefinition;
  };
  const exported = (mod.default ?? mod) as RailwayProgram | ProjectDefinition;
  const definition = await resolveDefinition(exported);
  const graph = projectDefinitionToGraph(definition);
  const desiredConfig = graphToEnvironmentConfig(graph, options);
  return { graph, desiredConfig };
}

async function resolveDefinition(exported: RailwayProgram | ProjectDefinition): Promise<ProjectDefinition> {
  if (typeof exported === "function") return exported(projectFactory, createRailwayContext());
  return exported;
}
