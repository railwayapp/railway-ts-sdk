...
`,
  ...
  try {
    await expect
      .poll(() => ports.every(port => stdout.includes(`ready:${port}`)), {
        timeout: 30_000,
        interval: 250,
      })
      .toBe(true);
...
describe.runIf(live)("domains e2e (live)", () => {
  ...
  it("publishes generated and explicit prefixes and preserves domain metadata", async () => {
    ...
    expect(
      (await Sandbox.list()).find(sandbox => sandbox.id === source.id)?.domains,
    ).toEqual(domains);
  ...
  it("does not inherit source domains and can publish new routes on a fork", async () => {
    ...
    expect(source.domains.map(domain => domain.domain)).not.toContain(
      exposed.domains[0]!.domain,
    );
  ...
  it.each([undefined, "ISOLATED"] as const)(
    ...
    async networkIsolation => {
      const creation = Sandbox.create({
        domains: [{ port: 8080 }],
        ...(networkIsolation === undefined ? {} : { networkIsolation }),
      }).then(track);
      await expect(creation).rejects.toBeInstanceOf(RailwayGraphQLError);
      await expect(creation).rejects.toThrow(/PRIVATE/);
    },
    60_000,
  );
});
